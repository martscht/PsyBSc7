---
title: "Sitzung 6: Regressionanalyse III --- Aufgaben mit Lösungen"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
data("PISA_2009_v2", package = "PsyBSc7")
knitr::opts_chunk$set(exercise.checker = gradethis::grade_learnr)
```

?????????? lm.beta auch einbauen in normale Sitzung !??!?!
...
## 1. Prüfen Sie die Annahme der Homoskedastizität 
...
## 2. Prüfen Sie die Annahme der Linearität 
...
## 3. Püfen Sie die Normalverteilung der Residuen 
### a) grafisch und 
```{r}
...
## 4. Prüfen Sie die Multikollinearität der Prädiktoren im Modell
### a) durch Inspektion der bivariaten Korrelationen und

```{r}
cor(PISA_2009[,c("HISEI", "MotherEdu", "Books")])
...
## 5. Ausreißer und einflussreiche Datenpunkte
...
round(PISA_2009[IDs,c("Reading", "HISEI", "MotherEdu", "Books")],2)
# z-Standardisierte Werte der auffälligen Fälle
round(scale(PISA_2009)[IDs,c("Reading", "HISEI", "MotherEdu", "Books")],2)

```

### Ergebnisinterpretation 5.

* Nach dem pauschalen Kriterium $4/n$ ist die Hälfte der Hebelwerte auffällig ($h_j>0.027$). * Nach dem großzügigsten Kriterium $3\cdot k / n$ liegen drei Fälle über der Grenze ($h_j>0.06$)
...
* `influencePlot` kennzeichnet vier Fälle als auffällig. Diese können wie folgt charakterisiert werden:
    * Fall 43: Ausreißer nach unten beim Abschluss der Mutter (kein Abschluss), sonst unauffällig
    ...
    * Fall 141: Niedrige Lesekompetenz "trotz" höchstem Abschluss der Mutter und maxiamal hoher Bücherzahl
...
```{r}
m1.c <- lm(Reading ~ HISEI + MotherEdu + Books, data = PISA_2009[-141,])
summary(m1)
summary(m1.c)
