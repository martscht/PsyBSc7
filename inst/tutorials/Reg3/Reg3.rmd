...
```{r test_stdreg-check}
grade_result(
  fail_if(~ (!identical(.result, summary(lm.beta(lm(reading ~ female + IQ, data = Schulleistungen))))), 'Haben Sie die richtigen Input-Argumente verwendet? Sie wollen die summary des standardisierten Modells ausgeben. Schauen Sie sich ggf. die Hints an.'),
  pass_if(~ identical(.result, summary(lm.beta(lm(reading ~ female + IQ, data = Schulleistungen))))),
