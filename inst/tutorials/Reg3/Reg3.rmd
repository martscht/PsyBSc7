...
Im Output sehen wir die Parameterschätzungen unseres Regressionsmodells: $$\text{Reading}_i=\beta_0+\beta_1\text{Female}_i+\beta_2\text{IQ}_i+\varepsilon_i,$$ für $i=1,\dots,100=:n$.
...
Für weiterführende Informationen zur Instabilität und Standardfehlern siehe im [Appendix A](#AppendixA) nach.
...
Offensichtlich genügt eine der beiden Statistiken, da sie vollständig redundant sind. Empfehlungen als Grenzwert für Kollinearitätsprobleme sind z. B. $VIF_j>10$ ($T_j<0.1$) ([Eid, Gollwitzer, & Schmitt, 2017](https://hds.hebis.de/ubffm/Record/HEB366849158)). Die Varianzinflationsfaktoren der Prädiktoren im Modell können mit der Funktion `vif` des `car`-Paktes bestimmt werden, der Toleranzwert als Kehrwert der VIFs.
...
Für eine einfache Regressionsgleichung mit $$Y_i=\beta_0 + \beta_1x_{i1} + \beta_{i2} + \varepsilon_i$$
...
wobei $\hat{\sigma}^2_e$ die Resiudalvarianz unserer Regressionanalyse ist (also der nicht-erklärte Anteil an der Varianz von $Y$). Aus der Matrix $I$ erhalten wir die Standardfehler sehr einfach: Sie stehen im Quadrat auf der Diagonale. Das heißt, die Standardfehler sind $SE(\beta)=\sqrt{\text{diag}(I)}$ (Wir nehmen mit $\diag$ die Diagonalelemente aus $I$ und ziehen aus diesen jeweils die Wurzel: der erste Eintrag ist der $SE$ von $\beta_0$; also $SE(\beta_0)=\sqrt{I_{11}}$; der zweite von $\beta_1$;$SE(\beta_1)=\sqrt{I_{22}}$; usw.). Was hat das nun mit der Kolinearität zu tun? Wir wissen, dass in $X'X$ die Information über die Kovariation im Datensatz steckt. Bspw. lässt sich die empirische Kovarianzmatrix $S$ zwischen zwei Variablen $z_1$ und $z_2$ sehr einfach bestimmen ($Z=(z_1, z_2)$: $S=\frac{1}{n}Z'Z - \begin{pmatrix}\overline{z}_1\\\overline{z}_2 \end{pmatrix}\begin{pmatrix}\overline{z}_1&\overline{z}_2 \end{pmatrix}$. Weitere Informationen hierzu (Kovarianzmatrix und Standardfehler) sind im Appendix B (sowie auch in einigen Kapiteln) von [Eid et al. (2017)](https://hds.hebis.de/ubffm/Record/HEB366849158) Unterpunkt 5.2-5.4 bzw. ab Seite 1058 nachzulesen. 

Insgesamt bedeutet dies, dass die Standardfehler von der Inversen der Kovarianzmatrix unserer Daten sowie von der Residualvarianz abhängen. Sie sind also groß, wenn die Resiudalvarianz groß ist (damit ist die Vorhersage von $Y$ schlecht) oder wenn die Inverse der Kovarianzmatrix groß ist. Nehmen wir dazu an, dass $\hat{\sigma}_e^2=1$. Wir gucken uns jetzt drei Fälle an mit 

*Fall 1*: $X'X=\begin{pmatrix}1&0&0\\0&4&0\\0&0&4\end{pmatrix}$.
*Fall 2*: $X'X=\begin{pmatrix}1&0.9&0.8\\3.9&4&4.1\\4.1&3.9&4\end{pmatrix}$  und 
*Fall 3*: $X'X=\begin{pmatrix}1&1&1\\4&4&4\\4&4&4\end{pmatrix}$.
...
solve(XX_1) # Inverse
sqrt(diag(solve(XX_1))) # Wurzel aus den Diagonalelementen der Inverse = SE, wenn sigma_e^2=1
```

Im *Fall 2* sind alle fast perfekt korreliert - es liegt hohe Multikolinearität vor. Die Inverse ist fast nicht zu bilden. Die Standardfehler sind deutlich erhöht im Vergleich zu Fall 1
```{r}
XX_2 <- matrix(c(1,0.9,0.8,
               3.9,4,4.1,
               4.1,3.9,4),3,3)
XX_2 # Die Matrix X'X im Fall 2
solve(XX_2) # Inverse
sqrt(diag(solve(XX_2))) # Wurzel aus den Diagonalelementen der Inverse = SE, wenn sigma_e^2=1
sqrt(diag(solve(XX_1))) # Wurzel aus den Diagonalelementen der Inverse = SE, wenn sigma_e^2=1
```

Im *Fall 3* sind alle perfekt korreliert - es liegt perfekte Multikolinearität vor. Die Inverse ist  **nicht** zu bilden. Die Standardfehler sind nicht zu berechnen. Eine Fehlermeldung wird ausgegeben.
```{r}
XX_3 <- matrix(c(1,1,1,
               4,4,4,
               4,4,4),3,3)
...
```{r, eval = F}
solve(XX_3) # Inverse
sqrt(diag(solve(XX_3))) # Wurzel aus den Diagonalelementen der Inverse = SE, wenn sigma_e^2=1

# hier wird eine Fehlermeldung ausgegeben, wodurch der Code nicht ausführbar ist:
