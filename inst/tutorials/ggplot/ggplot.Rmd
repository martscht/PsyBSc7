...
Dafür nutzen wir die Daten, die [von der Johns Hopkins Universität auf GitHub](https://github.com/CSSEGISandData/COVID-19) bereitgestellt werden. Für uns sind dabei die drei Datensätze zu [bestätigten Fällen](https://github.com/CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv), [Todesopfern](https://github.com/CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv) und [genesenen Patienten und Patientinnen](https://github.com/CSSEGISandData/COVID-19/blob/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Recovered.csv) zentral.
...
Die erste Möglichkeit die Daten zu beziehen, ist es sie als .csv Dateien herunterzuladen und lokal zu speichern. Dafür reichen diese drei Links aus: [`r fa('download')` Bestätigte Fälle](https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Confirmed.csv), [`r fa('download')` Todesfälle](https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Deaths.csv) und [`r fa('download')` genesene Fälle](https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_19-covid-Recovered.csv).
...
Bevor jetzt die drei Arten von Fallzahlen zusammenführen können, müssen auch `deaths` und `recovered` ins Long-Format übertragen werden. Sie können unten probieren, ob das Vorgehen zum gewünschten Ergebnis führt. Sie können auch Änderungen vornehmen, um zu sehen, wie sich das Ergebnis verändert.
...
Das Gleiche benötigen wir dann noch einmal für den Datensatz der genesenen Fälle:
...
Die drei Datensätze, die wir jetzt erzeugt haben bestehen jeweils aus vier Variablen zur Region (`Province.State`, `Country.Region`, `Lat` und `Long`) einer Variable zur Zeit (`Day`) und der Anzahl der entsprechenden Fälle (`Confirmed`, `Deaths` bzw. `Recovered`). Um die letzten drei in einem Datensatz zusammenzuführen können wir die ersten fünf nutzen um eindeutig festzulegen, welche Daten zusammengehören. Daten vom gleichen Ort zur gleichen Zeit gehören zusammen. Für das Zusammenführen von zwei Datensätzen gibt es in R den `merge` Befehl, der drei zentrale Argumente entgegennimmt:

  - `x`: Der erste Datensatz
  - `y`: Der zweite Datensatz
  - `by`: Die Variablen, anhand derer gleiche Fälle identifiziert werden können

In der Hilfe zur Funktion finden sich auch noch viele zusätzliche Argumente, aber diese Drei sind für uns zentral. Leider lassen sich immer nur zwei Datensätze (`x` und `y`) zusammenführen, sodass wir `merge` zwei mal ausführen müssen, um aus unseren drei Datensätzen einen zu machen:
...
Um nun aggregieren zu können, können wir `aggregate` eine Formel geben, wie wir sie schon für Regressionen im letzten Semester verwendet haben. Die generelle Struktur solcher Formeln ist immer `AV ~ UV`, wobei mehrere UVs durch `+` vebunden werden können. In unserem Fall behandeln wir die drei Spalten `Confirmed`, `Deaths` und `Recovered` als AVs und `Contry.Region` plus `Day` als UV. Zusätzlich benötigt der Befehl dann eine Auskunft darüber, auf welchen Datensatz wir uns beziehen und mit welcher Funktion wir aggregieren wollen:
...
Der Datensatz enthält jetzt bestätigte Fälle, Todesfälle und genesene Fälle pro Land und für jeden Tag und ist damit bereit für die Visualisierung.
...
```{r object_combos, exercise=TRUE, exercise.setup='basic_plot'}
basic + geom_point()
```
...
| Vietnam | 955 | .693 |
  ...
  4. Stellen Sie für jedes Land in einer Teilabbildung die bestätigten Fälle (`Confirmed`), die Todesfälle (`Deaths`) und die genesenen Fälle (`Recovered`) dar. Nutzen sie für die drei Linien unterschiedliche Farben (aber über alle Länder hinweg die gleichen). Weil sich die Bevölkerungszahlen so drastisch unterscheiden, lassen sie die y-Achse über die Länder hinweg frei variieren.
  5. Nutzen Sie ein Balkendiagramm, um die relative Häufigkeit genesener Personen (`Recovered`) and der Gesamtzahl besätigter Fälle (`Confirmed`) pro Land bis zum heutigen Tag darzustellen. Denken Sie daran, dass Sie dafür Daten aggregieren müssen. Füllen Sie die Balken dabei mit länderspezifischen Farben.
