---
title: "Regressionanalyse IV Modellerweiterungen und nichtlineare Effekte"
...
### Datensatz (PISA: wie in den Übungen zum letzten Termin) $*$
$*$ _zum Teil basierend auf Unterlagen von Prof. Johannes Hartig_
...
library(ggplot2)
# Datensatz laden 
PISA_2009 <- readRDS("PISA_2009_v2.rda")
```
...
In der letzten Sitzung hatten wir herausgefunden, dass der Sozialstatus (`HISEI`), der Bildungsabschluss der Mutter (`MotherEdu`) und die Zahl der Bücher zu Hause (`Books`) bedeutsame Prädiktoren für die Lesekompetenz der Schüler/innen sind, allerdings zeigten Analysen, dass nicht alle Voraussetzungen erfüllt waren:


```{r}
# Berechnung des Modells und Ausgabe der Ergebnisse
m1 <- lm(Reading ~ HISEI + MotherEdu + Books, data = PISA_2009)
summary(lm.beta(m1))
```
...
Wird für den Bildungsabschluss der Mutter mit der Funktion `poly` ein linearer und qudratischer Trend in das Regressionsmodell aufgenommen, wird der quadratische Trend signifikant und das Modell erklärt signifikant mehr Varianz als ohne den quadratischen Trend:

```{r}
# Modell mit linearem + quadratischem Trend für MotherEdu:
m1.b <- lm(Reading ~ HISEI + poly(MotherEdu, 2) + Books, data = PISA_2009)
...
```{r}
m1.b1 <- lm(Reading ~ HISEI + poly(MotherEdu, 1) + Books, data = PISA_2009)
summary(lm.beta(m1.b1))
m1.b2 <- lm(Reading ~ HISEI + poly(MotherEdu, 2) + Books, data = PISA_2009)
summary(lm.beta(m1.b2))

PISA_2009$MotherEdu2 <- PISA_2009$MotherEdu^2 # füge dem Datensatz den quadrierten Bildungsabschluss der Mutter hinzu
m1.c1 <- lm(Reading ~ HISEI + MotherEdu + Books, data = PISA_2009)
summary(lm.beta(m1.c1))
m1.c2 <- lm(Reading ~ HISEI + MotherEdu + MotherEdu2 + Books, data = PISA_2009)
...
Ein sehr wichtiges Vorhersagemodell zu Zeiten der Corona-Pandemie oder dem starken Bevölkerungswachstum ist das exponentielle Wachstum. Um dieses genauer zu verstehen, werden einige Grundlagen im Umgang mit Exponenten benötigt. Zum Beispiel muss man sich überlegen, welche Rate und welche Basis ein exponentieller Verlauf hat. Beispielsweise wächst $10^x$ deutlich schneller als $2^x$. 

### Exkurs: Exponenten und Logarithmen

Um eine solche Gleichung nach $x$ aufzulösen wird der Logarithmus verwendet, denn er ist die Umkehrfunktion zum Exponentiellen (siehe beispielsweise hier für eine Wiederholung zum [Logarithmus und Logarithmusrechenregeln; Pierce (2018)](http://www.mathsisfun.com/algebra/exponents-logarithms.html), [Exponenten; Pierce (2019)](http://www.mathsisfun.com/exponent.html) oder [Exponentenrechenregeln; Pierce (2018)](http://www.mathsisfun.com/algebra/exponent-laws.html)). In der Schule wird zunächst der Logarithmus zur Basis *10* gelehrt. So kann man die Gleichung $10^x = 100$ leicht auflösen: Dazu müssen beide Seiten logarithmiert (zur Basis *10*) werden $\log_{10}(10^x) = \log_{10}(100)$ und wir erhalten $x = 2$. Es muss allerdings nicht immer der richtige Logarithmus zur jeweiligen Basis sein. Eine wichtige Logarithmusrechenregel besagt, dass der Logarithmus zur Basis *a* leicht mit Hilfe des Logarithmus zur Basis *b* berechnet werden kann: 

$\log_{a}(x) = \frac{\log_{b}(x)}{\log_{b}(a)}$,     siehe bspw. [Logarithmus und Logarithmusrechenregeln; Pierce (2018)](http://www.mathsisfun.com/algebra/exponents-logarithms.html).

Da diese Gesetzmäßigkeit gilt, wird in der Mathematik und auch in der Statistik meist nur der natürliche Logarithmus (*ln; logarithmus naturalis*) verwendet. Die Basis ist hierbei  *e* ($\approx 2.718\dots$). Mit dieser Rechenregel sowie der Rechenregeln für Exponenten können wir jedes exponentielles Wachstum als Wachstum zur Basis *e* darstellen, denn

$a^x = e^{\text{ln}(a^x)}=e^{x\text{ln}(a)}$, z.B. ist $10^3=e^{\text{ln}(10^3)}=e^{3\text{ln}(10)}=1000$. 

Da sich durch *e* und *ln* alle exponentiellen Verläufe darstellen lassen, wird in der Mathematik häufig *log* als das Symbol für den natürlich Logarithmus verwendet; so ist es auch in R: ohne weitere Einstellungen ist `log` der natürliche Logarithmus und `exp` ist die Exponentialfunktion:
...
plot(x = x, y = f, type = "l", col = "blue") # plotte f(x) gegen x
...
plot(x = x, y = log(f), type = "l", col = "blue") # plotte ln(f(x)) gegen x
abline(v = 0, lty = 3) # y-Achse, v = 0 zeichnet eine vertikale Linie bei x = 0
...
Die Dokumentation des Datensatzes mit Datenquellen sind hier einzusehen: [gapminder.org-Dokumentationen](https://www.gapminder.org/data/documentation/gd003/).

```{r}
load("WorldPopulation.rda") # Bereits zusammengefasster Datensatz von 1800-2020
head(pop_data) # Datenstruktur
ggplot(data = pop_data, aes(x = Year, y = Population))+geom_point()
```
...
```{r}
ggplot(data = pop_data, aes(x = Year, y = Population))+
     geom_point()+geom_smooth(method="lm", formula = "y~x")         # plotte linearen Verlauf 
m_l <- lm(Population ~ Year, data = pop_data) # linearer Verlauf
summary(m_l)  
...
Obwohl ein linearer Verlauf sehr unwahrscheinlich wirkt, können hier bereits `r round(summary(m_l)$r.squared*100, 2)`% der Variation der Bevölkerungsdichte durch die Jahreszahl eklären. Wie der Grafik deutlich zu entnehmen ist, sind die Residuen in dieser Regressionanalyse stark abhängig von der Jahreszahl (negatives Residuum von von a. 1860-1970 und positive Residuen sonst; Wiederholung: $\varepsilon_i=Y_i-\hat{Y}_i$, wobei $\hat{Y}_i$ der vorhergesagte Wert ist). Auch wenn wir uns das zugehörige Histogramm der Residuen ansehen, widerspricht dieses der Annahme auf Normalverteilung. Vielleicht ist auch hier ein quadratischer Effekt versteckt? Wir möchten dem auf den Grund gehen und nutzen wieder die Funktion `poly`, um ein Polynom 2. Grades (eine quadratische Funktion) der Jahreszahl in unsere Analysen mit aufzunehmen.

```{r}
m_q <- lm(Population ~ poly(Year,2), data = pop_data) # quadratischer Verlauf
summary(m_q)
...
Nun wollen wir prüfen, ob nicht ein exponentieller Verlauf die Daten noch besser beschreibt. Dazu müssen wir zunächst die Weltpopulation logarithmieren und können anschließend ein lineares Modell verwenden.



```{r}
pop_data$log_Population <- log(pop_data$Population) # Logarithmus der Weltbevölkerung
m_log <- lm(log_Population ~ Year, data = pop_data) # lineares Modell mit log(y) als AV (logarithmische Skala)
summary(m_log)

pop_data$pred_Pop_exp <- exp(predict(m_log)) # Abspeichern der retransformierten vorhergesagten Werten (wieder auf der Skala der Weltbevölkerung)
...
Das lineare Modell für die logarithmierte Bevölkerunsdichte scheint gut zu den Daten zu passen. Insgesamt können `r round(summary(m_log)$r.squared*100, 2)` % der Variation in den Daten durch den Zeitverlauf erklärt werden; etwas weniger als durch das quadratische Wachstum, durch welches `r round(summary(m_q)$r.squared*100, 2)` % der Variation in den Daten durch den Zeitverlauf erklärt werden konnten. Das Diagramm der retransformierten vorhergesagten Werten signalisiert, dass ein exponentielles Wachstumsmodell die Daten gut beschreibt, allerdings scheint der quadratische Trend vor allem ab ca. 1975 die Daten besser zu beschreiben. Das Histogramm der Residuen des logarithmierten Modells spricht deutlich gegen die Normalverteilungsannahme. Um das Modell genauer zu untersuchen, schauen wir uns den Plot der logarithmierten Bevölkerungsdichte an: 

```{r}
ggplot(data = pop_data, aes(x = Year, y = log_Population))+
     geom_point()+geom_smooth(method="lm", formula = "y~x", col = "red")+      # plotte linearen Verlauf 
     geom_smooth(method="lm", formula = "y~poly(x,2)", col = "gold3")

residualPlot(m_log) # Residuenplots
```

Vielleicht wächst die Bevölkerung sogar schneller als exponentiell? Die gelbe Linie im logarithmierten Plot lässt dies vermuten. Auch die Residuen lassen vermuten, dass nichtlineare Effekte bestehen. Um dies genauer zu untersuchen fügen wir auch in das logarithmierte Modell einen quadratischen Trend der Zeit ein.


```{r}
m_log_quad <- lm(log_Population ~ poly(Year,2), data = pop_data)
summary(m_log_quad)
anova(m_log, m_log_quad)
pop_data$pred_Pop_exp_quad <- exp(predict(m_log_quad))
ggplot(data = pop_data, aes(x = Year, y = Population))+
     geom_point()+geom_smooth(method="lm", formula = "y~x")+         # plotte linearen Verlauf 
     geom_smooth(method="lm", formula = "y~poly(x,2)", col = "darkblue")+  # plotte quadratischen Verlauf
     geom_line(aes(x = Year, y = pred_Pop_exp), col = "red", lwd = 1.5)+
