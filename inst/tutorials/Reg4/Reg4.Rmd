...
Durch `+ geom_smooth(method="lm", formula = "y~x")`, kann mit `ggplot` ein linearer Trend inklusive Konfidenzintervall hinzugefügt werden. Obwohl ein linearer Verlauf sehr unwahrscheinlich erscheint, können mit dem linearen Modell bereits `r round(summary(m_l)$r.squared*100, 2)`% der Variation der Bevölkerungsdichte durch die Jahreszahl eklärt werden. Wie der Grafik deutlich zu entnehmen ist, sind die Residuen in dieser Regressionanalyse stark abhängig von der Jahreszahl (negatives Residuum von von a. 1860-1970 und positive Residuen sonst; *Wiederholung*: $\varepsilon_i=Y_i-\hat{Y}_i$, wobei $\hat{Y}_i$ der vorhergesagte Wert ist). Auch wenn wir uns das zugehörige Histogramm der Residuen ansehen, widerspricht dieses der Annahme auf Normalverteilung. 
...
grade_result(
  fail_if(~ (!identical(.result, res)), 'Sie müssen die Variable log_Population vorhersagen durch das Jahr. Geben Sie davon die summary aus.'),
  pass_if(~ identical(.result, res)),
  correct = 'Sehr gut! Sie haben die logarithmierte Weltbevölkerung durch den  Trend der Zeit vorhergesagt und die summary davon ausgegeben.',
  incorrect = 'Leider falsch.',
  glue_correct = '{.correct}',
  glue_incorrect = '{.incorrect} {.message}')
```
...
```{r test_log_Pop_reg_quad,  exercise = TRUE, exercise.eval = FALSE, exercise.setup = "exercise_setup5"}
m_log_quad <- 
```


```{r test_log_Pop_reg_quad-check}
# Intro into excercise
WorldPopulation$log_Population <- log(WorldPopulation$Population) # Logarithmus der Weltbevölkerung
...
res <- summary(m_log_quad)

grade_result(
  fail_if(~ (!identical(.result, res)), 'Sie müssen die Variable log_Population vorhersagen durch den quadratischen Verlauf des Jahres. Geben Sie davon die summary aus.'),
  pass_if(~ identical(.result, res)),
  correct = 'Sehr gut! Sie haben die logarithmierte Weltbevölkerung durch einen quadratischen Trend der Zeit vorhergesagt und die summary davon ausgegeben',
  incorrect = 'Leider falsch.',
  glue_correct = '{.correct}',
  glue_incorrect = '{.incorrect} {.message}')
```
...
```{r test_anova_pop_log,  exercise = TRUE, exercise.eval = FALSE, exercise.setup="exercise_setup2"}
...
#### **Aufgabe 3.** Prüfen Sie nun, ob sie die Vorhersage durch ein quadratisch-exponentielles Modell verbessern können


#### **Bonusaufgabe.**: Vergleichen Sie die Koeffizienten des exponentiellen Wachstums zwischen den Ländern (*Diese Aufgabe ist komplett freiwillig und muss nicht gelöst werden, um die Studienleistung zu bekommen.*)
