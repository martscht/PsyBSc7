...
```{r, ech= F, fig.align="center", fig.height=6}
X <- scale(poly(PISA2009$MotherEdu, 2))
std_par_ME <- c(0.1588, -0.1436)
pred_effect_ME <- X %*% std_par_ME
std_ME <- scale(PISA2009$MotherEdu)
plot(x = std_ME,  y = pred_effect_ME, pch = 16, col = "blue", cex = 2,
     ylab = "Leseleistung | Others", xlab = "Bildungsabschluss der Mutter | Others",
     main = "Standardisierte bedingte Beziehung zwischen\n Bildungsabschluss der Mutter und Leseleistung")
```
Die Grafik zeigt die vorhergesagte Beziehung zwischen den standardisierten Werten des Bildungsabschlusses der Mutter sowie der Leseleistung. Hierbei steht erneut `|` für gegeben (wie etwa beim Partialplot mit `avPlots` aus der vergangenen Sitzung: `PsyBSc7::Sitzung_7()`). Wir sehen also den um die anderen Variablen im Modell bereinigten Effekt zwischen Bildungsabschluss und Leseleistung. Hierbei ist ein starker mittlerer Anstieg der Leseleistung (eine Standardabweichung von ca. -1 bis ca. 0.1) für einen Anstieg des Bildungsabschlusses von deutlich unterdurchschnittlich bis durchschnittlich (ca. std. Bildungsabschluss von -2.5 bis 0) zu sehen. Danach fällt die Beziehung zwischen Leseleistung und Bildungsabschluss fast horizontal aus (Veränderung geringer als 0.1 Standardabweichungen der Leseleistung), was dafür spricht, dass für durchschnittlichen bis überdurchschnittlichen (std. Bildungsabschluss von 0 bis 1) Bildungsabschluss der Mutter es kaum eine Beziehung zwischen den beiden Variablen gibt. Dies bedeutet, dass besonders das Fehlen eines Bildungsabschlusses der Mutter drastische Auswirkungen auf die Leseleistung des Kindes hat, es aber fast keinen Unterschied mehr macht, wenn die Mutter einen besonders hohen Bildungsabschluss hat.
